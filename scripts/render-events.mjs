    ...
    <div class="wrap event-hero-grid">
      <div>
        ...
        <div class="event-actions">
          ...
          <a class="event-btn event-btn-secondary" href="#directo">${isFinished ? "Ver la emisión completa" : "Ver la emisión"}</a>
        </div>
      </div>
...
${isFinished ? `      <div class="event-browser reveal">
      ...
      <div id="event-panel-all" role="tabpanel" aria-labelledby="event-tab-all" data-event-panel="all" hidden>
        ...
        ${announcements || `<div class="event-live-empty reveal"><b>↻</b><div><strong>La cobertura comenzará con la emisión</strong><p>Esta sección se llenará con los anuncios, fechas y tráilers confirmados durante ${escapeHtml(data.shortTitle)}.</p></div></div>`}
      </div>`}
    </section>
...
`}
    ...
    <section class="event-section" id="directo">
      <div class="event-section-heading reveal"><div><span class="event-kicker">Retransmisión oficial</span><h2>${isFinished ? "Volver a ver" : "Ver"} ${escapeHtml(data.shortTitle)}</h2><p>${isFinished ? "La emisión completa permanece disponible en el canal oficial." : "La emisión oficial puede verse sin salir de la cobertura."}</p></div></div>
