...
- Al finalizar la emisión, conservar el archivo completo en `announcements` y sustituir la cronología por dos vistas: una selección de `highlights` ordenada de mayor a menor relevancia y un archivo alfabético buscable. Los filtros se definen mediante `archiveStages` para reflejar las partes reales de cada evento y cada anuncio puede declarar uno o varios `stages`. El resumen final no muestra horas.
- Cada entrada del archivo completo incluye una imagen ligera: por defecto se usa la miniatura de su tráiler de YouTube y `archiveImage` permite sustituirla por arte oficial cuando el vídeo apunta a la retransmisión completa.
