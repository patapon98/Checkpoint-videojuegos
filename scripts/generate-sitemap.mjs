...
const PUBLIC_PAGE = /^(?:index|noticias|resenas|calendario|juegos|playstation-plus|sobre-mi|contacto|privacidad)\.html$|^(?:noticias|resenas|playstation-plus|juegos|eventos)\/[^/]+\.html$/;
...
function pageOrder(page) {
  ...
  const fixed = new Map([["/", 0], ["/noticias", 1], ["/resenas", 5], ["/calendario", 7], ["/playstation-plus", 8], ["/sobre-mi", 10], ["/contacto", 11], ["/privacidad", 12]]);
  if (fixed.has(pathname)) return [fixed.get(pathname), pathname];
  if (pathname.startsWith("/noticias/")) return [2, pathname];
  if (pathname.startsWith("/eventos/")) return [3, pathname];
  if (pathname.startsWith("/juegos/")) return [4, pathname];
  if (pathname.startsWith("/resenas/")) return [6, pathname];
  if (pathname.startsWith("/playstation-plus/")) return [9, pathname];
  return [13, pathname];
}
