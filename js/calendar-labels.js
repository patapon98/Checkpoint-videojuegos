(function(){
  ...
  if(doomRelease){
    ...
    if(image){ image.src=listArt; image.alt='DOOM: The Dark Ages | Revelations'; image.classList.remove('fit-contain'); }
  ...
  releases.forEach(release=>{
    ...
    if(heading.querySelector('.tag-dlc')){ badge.remove(); return; }
    if(eventOfYear.has(title)){ badge.className='hype hype-max'; badge.textContent='El evento del año'; return; }
    if(highHype.has(title)){ badge.className='hype hype-high'; badge.textContent='Hype alto'; return; }
    if(interesting.has(title)){ badge.className='hype hype-mid'; badge.textContent='Interesante'; return; }
    badge.remove();
  });
})();